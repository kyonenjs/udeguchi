...
const download_simple_quiz = async (quiz_id, quiz_path, auth_headers) => {
	...
	const quizzesHTML = quizzes.map((quiz, i) => {
		...
		const answers = quiz.prompt.answers.map((answer, answer_index) => {
			return `<div class="radio"><span class="badge">${answer_index + 1}</span> <label><input type="radio" name="answer">${answer}</label></div>`
		}).join('');
		...
		return `<div class="panel panel-info"><div class="panel-heading"><h4 class="panel-title"><a role="button" data-toggle="collapse" href="#question${currentIndex}">Question ${currentIndex}</a></h4></div><div id="question${currentIndex}" class="panel-collapse collapse${i === 0 ? ' in' : ''}"><div class="panel-body"><div class="panel panel-default"><div class="panel-heading"><h4 class="panel-title">${quiz.prompt.question}</h4></div><div class="panel-body">${answers}</div></div><div class="panel panel-success"><div class="panel-heading"><h4 class="panel-title"><a class="collapsed" data-toggle="collapse" href="#answer${currentIndex}">Correct Answer</a></h4></div><div id="answer${currentIndex}" class="panel-collapse collapse"><div class="panel-body"><span class="badge">${correct_answer_position + 1}</span> ${correct_answer}</div></div></div></div></div></div>`
	}).join('');
